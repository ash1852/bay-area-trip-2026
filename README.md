...
- 默认显示一级「景点 / 地点」和「中转」；节点旁的 `+ / −` 单独展开与收起，点击主体查看详情。
- 地点包含连续停留的活动。同址早晚不同停留分开；`places.<id>.parentPlace` 可将岛上码头、园内咖啡厅等归入所属景点。
- 连续外部移动合为一个中转，显示总体起止时间与时长。展开后查看具体交通；中转内部时刻未提供时保留待细化。
- `data/hierarchy.js` 从行程生成视图，不维护第二份行程。可在 `move` 的 extra 中填写 `legs: [{from,to,mode,title,start:null,end:null,detail}]` 来细化方案。两站点 `via` 的跨湾 BART 路线目前自动分成入站、轨道、出站三段；更复杂路线优先填写 `legs`。
- 地图、左侧行程和时间轴共享展开状态，链接保留日期、模式、选中节点和展开状态；旧活动链接继续有效。
- 地图标签会避让，细虚线指向真实坐标，彩色实线连接父子标签。交通虚线仍是连接示意，并非道路轨迹。
